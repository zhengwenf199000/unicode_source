    ...
    createElementVNode("view", { class: "content" }, [
      ...
      createElementVNode("view", { class: "content-c daily_content" }, [
        createElementVNode("view", { class: "daily_content" }, [
          ...
          createElementVNode("u-text", { class: "daily_content-status" }, toDisplayString($data.name) + " 您好，" + toDisplayString(!$data.success && !$data.failure ? "正在签到..." : "") + toDisplayString($data.success ? "您已成功签到！" : "") + toDisplayString($data.failure ? "签到失败，请重试！" : ""), 1)
        ]),
        $data.success || $data.failure ? (openBlock(), createElementBlock("u-text", {
          key: 0,
          class: "daily_content-timeout"
        }, toDisplayString($data.leftSecond) + " 秒后自动关闭", 1)) : createCommentVNode("", true)
      ])
    ])
  ]);
}
const daily = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["styles", [_style_0]]]);
export {
  daily as default
};
